...
def process_json(extracted_info, input_path, client):
    ...
    for info in extracted_info:
        ...
        for entity in data['entities']:
            same_entity_type = next(
                (e for e in new_dict['extractions']['entities'] if e['type'] == entity['type']), None
            )
            if same_entity_type and entity['type'] == "Testator":
                add_or_update_entities(entity_id_map, new_dict, full_text_len, data, entity, same_entity_type, sentence)
            elif same_entity_type and entity['type'] not in {"Condition", "Time"}:
                # for gpt-assisted coreference resolution:
                gpt_coref_res = gpt_assisted_coreference_resolution.main(entity, new_dict, client)
                add_or_update_entities(entity_id_map, new_dict, full_text_len, data, entity, gpt_coref_res if gpt_coref_res else None, sentence)
                # for spacy-based coreference resolution:
                # coref_res = x_sentence_coref_res(new_dict['extractions']['entities'], entity, new_dict['full_text'])
                # prev_entity = next((entity for entity in new_dict['extractions']['entities'] if entity['id'] == coref_res), None)
