...
def process_json(extracted_info, input_path):
    sentence = 0
    new_dict = {}
    new_dict['file_name'] = input_path
    new_dict['testator_name'] = ""
    new_dict['execution_date'] = ""
    new_dict['full_text'] = ""
                                ...
                                for key, value in list(prev_entity['texts'].items()):
                                    if text == key:
                                        ...
                                        for offset in char_offsets:
                                            ...
                                            id_char = {}
                                            id_char["sentence_id"] = sentence
                                            id_char["character_offsets"] = global_offset
    ...
    for entity in new_dict['extractions']['entities']:
        ...
        if entity['type'] == "Testator":
            for key in entity["texts"].keys():
                # AK: This only works for the wills that are redacted with the special tokens. For actual wills, we'll need a way to find the testator's name.
                if key[0] == "[":
                    new_dict['testator_name'] = key
    return new_dict
