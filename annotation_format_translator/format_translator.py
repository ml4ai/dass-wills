...
"""
This program is for switching the format of the annotations downloaded from LabelStudio into the one used by our project.
To run this program, run the following code: python format_translator.py path/to/your/input_file.json path/to/your/output_path
"""
...
def main():
  ...
  for datum in data:
    ...
    # # take care of "inclusion" relations -- enable this if needed
    # relation_list = handling_relations(datum['annotations'][0]['result'], entity_dict)
    # relation_json_list = switch_relation_to_json(relation_list, datum['annotations'][0]['result'], id_dict)
    ...
    # enable this if there's any "inclusion" relations
    # final_json = {"text": datum['data']['text'], "entities": entity_json_list, "relations": relation_json_list, "events": event_json_list}
...
def handling_relations(data, final_entities):
  include_relations = []
  for item in data:
    entities = {}
    if item.get('labels') == ["INCLUSION"]:
      entities["includer"] = item['from_id']
      entities["includee"] = item['to_id']
      include_relations.append(entities)
  if len(include_relations) != 0:
    for relation in include_relations:
      for entity in final_entities.values():
        if relation["includer"] in entity:
          relation["includer"] = entity
        if relation["includee"] in entity:
          relation["includee"] = entity
  return include_relations

def switch_relation_to_json(relations_list, id_dict):
  relations_list = relation_convert_to_json_id(relations_list, id_dict)
  merged_relations = merge_relations(relations_list)

  final_relation_list = []
  for relation in merged_relations:
      relation_json = {}
      id = "r"+str(len(final_relation_list)+1)
      relation_json["id"] = id
      relation_json["type"] = "Inclusion"
      relation_json["includer"] = relation
      relation_json["includee"] = merged_relations[relation]
      final_relation_list.append(relation_json)
  return final_relation_list

def relation_convert_to_json_id(relations_list, id_dict):
  for relation in relations_list:
    for i in id_dict:
      if relation['includer'] == id_dict[i]:
        relation['includer'] = i
      if relation['includee'] == id_dict[i]:
        relation['includee'] = i
    if isinstance(relation['includer'], list):
      relation['includer'] = relation['includer'][0]
    if isinstance(relation['includee'], list):
      relation['includee'] = relation['includee'][0]
  return relations_list

def merge_relations(relations_list):
  new_dict = {}
  for relation in relations_list:
    if relation['includer'] not in new_dict:
      new_dict[relation['includer']] = [relation['includee']]
    elif relation['includer'] not in new_dict:
      new_dict[relation['includer'][0]] = [relation['includee']]
    else:
      new_dict[relation['includer']].append(relation['includee'])
  return new_dict
