...
def cmd_line_invocation():
    parser = argparse.ArgumentParser(description="""Retrieve a will given a testator ID and perform the following actions:\n - retrieve directives \n - validate thier conditions \n - execute the validated directives.""")
    parser.add_argument('-p', '--path-to-will', type=str, required=True, help="Input path to the will's [pickle] object.")
...
def execute_directive(directive,asset):
    ...
    print(f"... [STUB] Transferring '{asset}' to {directive['then']['beneficiary']['name']} with id {directive['then']['beneficiary']['id']}.")
...
def main():
    ...
    for directive in will_object._data._directives:
        if (validate_directive(will_object._data._directives[directive],directive)):
            validated_directives.append((will_object._data._directives[directive],directive))
