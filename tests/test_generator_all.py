...
def test_output(inp):
    (cmd_run_test_driver,base_dir) = inp
    print(f"Processing test: {cmd_run_test_driver[3]}")
...
def main():
    ...
    tests_passed = 0
    test_driver_script =  os.path.abspath(os.path.join(base_dir, 'test_generator.py')) 
    ...
    for i in range(TOTAL_TESTS):
        test_no = i + 1
        test_folder_name = f"will_{test_no}"
        # print(f"Processing test: {test_no}")
        test_folder_path = os.path.abspath(os.path.join(base_dir,'mass_testing',test_folder_name)) 
        ...
        inp = (cmd_run_test_driver, base_dir)
        result = test_output(inp)
